...
{
    public class Type1bFile : TFFile
    {
        private class ItemData
        {
            ...
            public TFString Data3;
        }

        private List<ItemData> _strings;

        public Type1bFile(string fileName) : base(fileName)
        {
        ...
        public override string FileType => "Spellforce 2 Type 1b Text File";
        ...
        public override void Read(Stream s)
        {
            ...
            for (var i = 0; i < numElems; i++)
            {
                ...
                if (dataLength > 0)
                {
                    str.Data3 = ReadString(s, dataLength * 2, $"{str.ItemId:0000}-{str.Unknown1:X4}");
                }
            ...
            ExportOptions options)
        {
            using (var fs = new FileStream(fileName, FileMode.Create))
            {
                ...
                foreach (var item in _strings)
                {
                    ...
                    if (item.Data3 != null)
                    {
                        var tfString = strings.FirstOrDefault(x => x.Offset == item.Data3.Offset);
