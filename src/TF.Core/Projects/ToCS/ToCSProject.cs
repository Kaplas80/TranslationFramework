...
using TF.Core.Projects.ToCS.Files;

namespace TF.Core.Projects.ToCS
{
    public class ToCSProject : Project
    {
        public override string CompatibleFilesFilter => "Archivos de Texto de ToCS|*.tbl";
        ...
        protected ToCSProject(Repository repository)
        {
            _repository = repository;
        }

        public static ToCSProject GetProject(string path)
        {
            ...
            var result = new ToCSProject(repository) {Path = path};
            return result;
        }
