...
{
    public class CommonShopBinFile : TFFile
    {
        public CommonShopBinFile(string fileName) : base(fileName)
        {
        ...
        public override string FileType => "common shop .bin";
        ...
        public override void Save(string fileName, byte[] originalContent, IList<TFString> strings, ExportOptions options)
        {
            using (var fs = new FileStream(fileName, FileMode.Create))
            {
                ...
                fs.WriteValueS32(0, Endianness);

                var currentOffset = 12 + 4 * strings.Count + _remainder.Length;
